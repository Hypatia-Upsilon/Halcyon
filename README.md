...
- 本地 MV 支持横屏播放、辉光进度条、截图分享、可拖动/锁定字幕、KTV 歌词、伴奏测试以及 LunaBeat `mv_offsets.json` 字幕偏移。
...
- **本地 MV**：显示视频缩略图与时长，点击后使用 Halcyon 内置播放器有声播放。
- **网易云 MV**：当歌曲的 `163 key` 含非零 `mvid` 时显示，点击跳转 `https://y.music.163.com/m/mv?id=<mvid>`。

若两者都存在，会同时显示两个入口；网易云 MV 不会替代本地 MV，也不会参与播放页静音同步。
...
> MKV 是容器格式，实际能否播放还取决于设备对其中视频编码的解码能力；常见 H.264、H.265 和 VP9 通常可以直接播放。
